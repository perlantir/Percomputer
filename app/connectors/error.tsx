...
export default function ConnectorsError({
  ...
  useEffect(() => {
    console.error("Connectors route error:", error);
  }, [error]);
  ...
  return (
    <div className="flex min-h-[60dvh] flex-col items-center justify-center gap-6 px-4">
      ...
      >
        <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-[var(--semantic-info)]/10">
          <Plug className="h-8 w-8 text-[var(--semantic-info)]" />
        </div>
        <div className="space-y-2">
          ...
          <p className="max-w-sm text-sm text-[var(--text-secondary)]">
            {error.message || "Could not load the connectors page."}
          </p>
          {error.digest && (
