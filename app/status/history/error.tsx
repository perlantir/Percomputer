...
export default function StatusHistoryError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("Status history route error:", error);
  }, [error]);
    ...
    <div className="flex min-h-[60dvh] flex-col items-center justify-center gap-6 px-4">
      ...
      >
        <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-[var(--accent-primary)]/10">
          <Activity className="h-8 w-8 text-[var(--accent-primary)]" />
        </div>
        <div className="space-y-2">
          <h2 className="text-lg font-semibold text-[var(--text-primary)]">
            Status history failed to load
          </h2>
          <p className="max-w-sm text-sm text-[var(--text-secondary)]">
            {error.message || "Could not load the status history page."}
          </p>
          {error.digest && (
